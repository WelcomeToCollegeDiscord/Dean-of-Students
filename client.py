...
class FrameworkClient(discord.Client):
    ...
    async def on_ready(self):
        ...
        log.info(
            f"Bot process ID is {os.getpid()}. If no_bpid_prefix is disabled this specific instance can be addressed with bpid{os.getpid()}.")
    ...
    async def on_reaction_add(self, reaction: discord.Reaction, source: Union[discord.User, discord.Member]):
        for func in self._reaction_add_handlers:
            try:
                ...
                log.warning("Ignoring exception in reaction_add coroutine (see stack trace below)",
    ...
    async def on_reaction_remove(self, reaction: discord.Reaction, source: Union[discord.User, discord.Member]):
        for func in self._reaction_remove_handlers:
            try:
                ...
                log.warning("Ignoring exception in reaction_remove coroutine (see stack trace below)",
                            include_exception=True)
    ...
    async def on_member_join(self, member: discord.Member):
        for func in self._member_join_handlers:
            try:
                ...
                log.warning("Ignoring exception in member_join coroutine (see stack trace below)",
                            include_exception=True)
    ...
    async def on_member_remove(self, member: discord.Member):
        for func in self._member_remove_handlers:
            try:
                ...
                log.warning("Ignoring exception in member_leave coroutine (see stack trace below)",
                            include_exception=True)
        ...
        def inner_decorator(func: Callable[[str, discord.Message], None]):
            ...
            async def new_cmd(command: str, message: discord.Message) -> None:
                try:
                    ...
                    embed = discord.Embed(title="Internal error",
                                          description=f"There was an error processing the command. Here's the stack trace, if necessary (this is also recorded in the log):\n```{stackdump}```",
                                          colour=0xf00000)
            ...
            log.debug(
                f"registered new command handler {func.__name__}() with trigger '{trigger}' and aliases: {aliases}")
            return new_cmd
    ...
    def basic_help(self, title: str, desc: str, include_prefix: bool = True):
        ...
        if title.strip() == "" or desc.strip() == "":
            log.critical(
                "Blank content in help message: running the help command with this blank content *will* throw an error. Traceback logged under debug level.")
